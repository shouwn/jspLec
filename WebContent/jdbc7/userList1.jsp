...
<form class="form-inline">
  <div class="form-group">
    ...
    <select name="od" class="form-control">
      ...
      <option value="1" <%= "1".equals(od) ? "selected" : "" %>>유저 아이디</option>
      <option value="2" <%= "2".equals(od) ? "selected" : "" %>>이름</option>
      <option value="2" <%= "3".equals(od) ? "selected" : "" %>>학과</option>
    </select>
    <select name="ss" class="form-control">
      <option value="0" <%= "0".equals(ss) ? "selected" : "" %>>전체</option>
      <option value="1" <%= "1".equals(ss) ? "selected" : "" %>>유저 아이디</option>
      <option value="2" <%= "2".equals(ss) ? "selected" : "" %>>이름</option>
      <option value="3" <%= "3".equals(ss) ? "selected" : "" %>>학과명</option>
    </select>
...
<table class="table table-bordered table-condensed">
  ...
  <tbody>
    <% for (User user : list) { %>
      <tr data-url="userEdit1.jsp?id=<%=user.getId()%>&pg=<%=currentPage%>&srchText=<%=stEncoded%>">
        <td><%= user.getId() %></td>
